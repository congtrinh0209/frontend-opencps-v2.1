    ...
    <v-card-text class="px-0 pt-0 pb-0">
      <v-row>
        <v-layout wrap class="mt-0">
          <v-flex
            class="px-0 xs12 md12"
            v-if="menuActive === 0 || menuActive === 3"
          >
          ...
          <v-flex
            class="px-0 xs12 md12"
          >
            <div class="mb-1">Số hiệu hồ sơ</div>
            <v-text-field
              class="search-input-appbar input-search"
              solo
              flat
              clearable
              v-model="dataSearch['soHieuGiayTo']"
            ></v-text-field>
          </v-flex>
              ...
              <v-date-picker
                :min="
                  fromReceiveDateFormatted
                    ? getMinMax(fromReceiveDateFormatted)
                    : null
                "
                :max="currentDate()"
      ...
      <v-flex class="text-right">
        ...
        >
          <v-icon size="20"> search </v-icon> &nbsp; Tìm kiếm
        </v-btn>
      </v-flex>
    </v-card-text>
  </div>
...
export default {
  name: "Search",
  props: ["form", "inputSearch", "menuActive"],
  data() {
    return {
      donViList: [],
      fileTemplateList: [],
  ...
  watch: {
    $route: function (newRoute, oldRoute) {
      ...
      if (
        currentQuery.hasOwnProperty("applicantIdNo") &&
        currentQuery.applicantIdNo
      ) {
